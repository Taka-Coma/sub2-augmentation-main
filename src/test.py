...
tree_id = random.randint(0, 10 - 1)
print(tree_id)


for line in open("data/sst/test.txt"):
	print(line)
...
# #     data = list()
# #     for path in glob('data/conllu/*conllu',):
# #         cnt = 0
# #         line_sum = sum([1 for _ in open(path)])
# #         for line in open(path):
# #             cnt = cnt + 1
# #             print(path,"...........",cnt,"/",line_sum)
# #             if is_data_line(line):
# #                 current_conll_data.append(line.strip().split('\t'))
# #             elif len(current_conll_data) > 0:
# #                 data.append(Sentence(current_conll_data))
# #                 current_conll_data = list()

# #         if len(current_conll_data) > 0:
# #             print("last if ...",current_conll_data)
# #             data.append(Sentence(current_conll_data))

    
# #     print("data",data)

# #     with open("data2.pickle", mode="wb") as f:
# #         pickle.dump(data, f)
